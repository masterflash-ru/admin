...
public function save()
{
    ...
    if (!empty($_FILES["zip_".$this->col_name]["name"][$this->id])){
        ...
        if (!is_dir($_SERVER['DOCUMENT_ROOT']."/media/banner/".$v['folder'])) {
            mkdir($_SERVER['DOCUMENT_ROOT']."/media/banner/".$v['folder'],0777);
        }
        
        
        if ($_FILES["zip_".$this->col_name]["error"][$this->id] == UPLOAD_ERR_OK) {
            $zip_file=md5(1000+microtime(true));
            if (!move_uploaded_file( $_FILES["zip_".$this->col_name]["tmp_name"][$this->id],$path."/".$zip_file.".zip")) {
            ...
            if ($zip->open($zip_file) === TRUE) {
                ...
                if (count($d)>0){
                    ...
                    if (isset($meta["ad_size"])){
                        ...
                        echo "Нет метатега ad_size ";
                        return serialize([]);
                    }
                } else {
                    echo "в архиве нет файла html";
                    return serialize([]);
                }
            } else {
                echo 'ошибка разархивации, возможно файл не является ZIP архивом';
                return serialize([]);
            }
        }
