...
public function render()
{
	...
	for ($i=0;$i<count($interface_type);$i++){
		...
		'?'.$this->properties['get_name'].'='.$this->value.'&get_interface_input='.rawurlencode(base64_encode(serialize($this->value))).
		'&window_close_type='.$this->properties['window_close_type'].
		'","","'.str_replace(' ',',',$this->properties['window_properties']).'");return false;\'';
        
        $m="/adm/". $_url. $interface_name[$i].'?get_interface_input='.rawurlencode(base64_encode(serialize($this->value))).'&'.$this->properties['get_name'].'='.$this->value.
		'&window_close_type='.$this->properties['window_close_type'];
